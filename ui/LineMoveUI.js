class LineMoveUI {
		...
		this._mouseDownEventListener = event => {
            if (!this._moving) {
                ...
                if (this._end_point == null) {
                    ...
                    this._fireEvent("moveEnded", new Line(this._start_point, this._end_point, this._chosen_line.color.slice()));
                }
                this.reset();
            }
		};
    ...
    getLine(objectList) {
        ...
        for (var i = objectList.length - 1; i >= 0; i--) {
            if (objectList[i] instanceof Line) {
                if (objectList[i].p1.distance(pointChosen) < tolerance || 
					objectList[i].p2.distance(pointChosen) < tolerance) {
                    this._chosen_line = objectList[i];
                    (objectList[i].p1.distance(pointChosen) < objectList[i].p2.distance(pointChosen) ? 
	...
	getHelperObjects() {
		if (this._end_point != null) {
			return [new Line(this._start_point, this._end_point, 
