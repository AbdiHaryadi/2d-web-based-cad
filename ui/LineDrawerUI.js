class LineDrawerUI {
		...
		this._mouseDownEventListener = event => {
			if (this._drawing) {
				if (this._end_point == null) {
					this._fireEvent("lineAborted", null);
				} else {
					this._fireEvent("lineCreated", new Line(this._start_point, this._end_point, this.color.slice()));
				}
	...
	getHelperObjects() {
		if (this._end_point != null) {
			return [new Line(this._start_point, this._end_point, this.color)];
		} else {
			return [];
		}
	}
